...
export default function LoginForm({ isLoggedIn, setIsLoggedIn, onChange }) {
	...
	const checkIfUserExists = async (email) => {
		...
		if (!results) {
			//change message
			console.log(
				'%cUSER DOES NOT EXIST',
				'font-size:1.25rem;font-weight:bold;color:red'
			);
			let tl = gsap.timeline({});
			tl.set(newButtonRef.current, { transformOrigin: 'center' }).to(
				newButtonRef.current,
				{ borderColor: 'rgb(187 247 208)', scale: 1.25, duration: 0.15 }
			);
			tl.to(newButtonRef.current, {
			...
			console.log(
				'%cUSER EXIST',
				'font-size:1.25rem;font-weight:bold;color:green'
			);
		}
	};
	...
	const checkCombination = async (combination) => {
		if (combination.length === 4) {
			...
			if (results.comboStr !== comboStr) {
				let failAttemptMessageArr = [
					'Nope, you lose',
					'Sorry, please try again',
					'Nuh-uh-uha',
				];
				let pickFailAttemptMessage = () =>
					failAttemptMessageArr[
						Math.floor(Math.random() * failAttemptMessageArr.length)
					];
				...
				setCombination([]);
				cookieCutter.set("UserLoggedIn",isLoggedIn)
			} else {
				setIsLoggedIn(true);
				console.log(
					'%cWELCOME %o',
					'color:yellow;font-size:1.34rem;',
					email.split('@')[0]
				);
				setIsLoggedIn(true);
				cookieCutter.set("UserLoggedIn",isLoggedIn)
	...
	const loginAttemptTracker = () => {
		if (loginAttempt === 3) {
			...
			console.log(
				"%cThird try, I'm sorry. Come back in like...30min",
				'font-size:1.35rem;color:red;'
			);
		} else {
	...
	return (
		<>
			<div
				...
				className="h-1/2 inset-96 transform-origin translate-y-1/2 flex flex-col justify-center items-center "
			>
				<div
					id="text-above-form"
					className="relative top-3 right-44 bg-transparent text-green-500 font-bold text-6xl"
				>
					&#60; IMGS+ &#62;
				</div>
				<div
					id="login-wrapper"
					className="SECTION 2xl:w-[600px] sm:w-[360px] h-4/5 flex flex-column justify-center items-center rounded-lg"
				>
					...
					>
						<div className="w-4/5 flex BLUE">
							...
							<div className="w-[100px] flex justify-center items-center">
								<button
									...
									ref={newButtonRef}
									className="flex justify-center items-center w-5/6 h-5/6 bg-transparent text-white rounded-xl border-t-4 border-l-4 border-r-4 border-b-2 border-green-500 hover:border-t-4 hover:border-l-4 hover:border-r-4 hover:border-b-2  hover:border-green-500 transition hover:shadow-[inset_0_5px_10px_#22c55e]"
								>
									NEW
								</button>
							</div>
						...
						<div
							id="buttons-group"
							data-test="login-buttons-group"
							className="flex gap-x-0 w-4/5 text-center "
						>
							<button
								id="button-1"
								ref={addButtonsRefs}
								...
								className="grow h-28 border-t-4 border-l-4 border-r-1 border-b-2 rounded-xl border-green-500 transition hover:shadow-[inset_0_5px_10px_#22c55e]"
							>
								b33p
							</button>
							<button
								id="button-2"
								ref={addButtonsRefs}
								data-string="0pt10nII"
								data-test="login-button"
								onClick={updateCombination}
								className="grow h-28 border-t-4 border-l-2 border-r-1 border-b-2 rounded-xl border-green-500 transition hover:shadow-[inset_0_5px_10px_#22c55e]"
							>
								bo0p
							</button>
							<button
							...
							<button
								...
								data-test="login-button"
								onClick={updateCombination}
								className="grow h-28 border-t-4 border-l-2 border-r-4 border-b-2 rounded-xl border-green-500 transition hover:shadow-[inset_0_5px_10px_#22c55e]"
							>
								MF&apos;r
							</button>
						</div>
